...
type PaintedPanel = (RobotPos, Color)
...
colorAt pos painted = case (find (\(p, c) -> p == pos) painted) of
  Just (_, c) -> c
  Nothing -> 0
...
paint :: RunningProgram -> [PaintedPanel]
paint p = paintR p [] (0,0) UpD
  where
    paintR prog panels pos direction
      ...
      where
        (newProg, output) = stepRobot prog $ colorAt pos panels
        [newColor, t] = output
        newPanels = panels ++ [(pos, newColor)]
        newDir = turn t direction
        newPos = move newDir pos

prog1 = RunningProgram { position = 0, program = input1, inputs = [], relativeBase = 0 }
paint1 = paint prog1
--answer1 = length $ Map.fromList paint1
answer1 = length $ filter (\(p,c) -> c == 1) paint1
