...
parseInput i = [parseOneMovement m | m <- (splitOn "," i)]

parseOneMovement :: String -> Movement
parseOneMovement (dir : dis) = (dir, (read dis))

inputA = parseInput inputAs
inputB = parseInput inputBs

crosses :: [Movement] -> Point -> Bool
crosses _ (0, 0) = True
crosses [] _ = False
crosses ((dir, dis) : ms) (x, y)
  | dir == 'U' && x == 0 && y > 0 && y <= dis = True 
  | dir == 'D' && x == 0 && y < 0 && y >= -dis = True 
  | dir == 'L' && y == 0 && x < 0 && x >= -dis = True 
  | dir == 'R' && y == 0 && x > 0 && x <= dis = True
  | dir == 'U' = crosses ms (x, y - dis) -- point is relative to the current position
  | dir == 'D' = crosses ms (x, y + dis)
  | dir == 'L' = crosses ms (x + dis, y)
  | dir == 'R' = crosses ms (x - dis, y)

move :: Movement -> Point -> Point
move ('U', dis) (x, y) = (x, y + dis)
move ('D', dis) (x, y) = (x, y - dis)
move ('L', dis) (x, y) = (x - dis, y)
move ('R', dis) (x, y) = (x + dis, y)

toPoints :: [Movement] -> [Point]
toPoints ms = toPointsR (0, 0) ms

toPointsR :: Point -> [Movement] -> [Point]
toPointsR _ [] = []
toPointsR p ((_, 0) : ms) = toPointsR p ms
toPointsR p ((dir, dis) : ms) = newPos : (toPointsR newPos ((dir, dis - 1) : ms))
                               where newPos = move (dir, 1) p
...
crossings :: [Movement] -> [Movement] -> [Point]
crossings a b = intersect (toPoints a) (toPoints b)

manhattan :: Point -> Point -> Distance
manhattan (x1, y1) (x2, y2) = (abs (x1 - x2)) + (abs (y1 - y2))

answer1crossings = crossings inputA inputB
answer1distances = [manhattan (0,0) x | x <- answer1crossings]
answer1 = minimum answer1distances

wireDistance :: Point -> [Movement] -> Distance
wireDistance (0, 0) _ = 0
wireDistance p ((_, 0) : ms) = wireDistance p ms
wireDistance p ((dir, dis) : ms) = 1 + (wireDistance relNewPos ((dir, dis - 1) : ms))
                                   where relNewPos = move (dir, -1) p

answer2distances = [(wireDistance x inputA) + (wireDistance x inputB) | x <- answer1crossings]
answer2 = minimum answer2distances
